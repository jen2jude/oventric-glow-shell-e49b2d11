 ...
 * Wallet funding.
 *
 * MVP lock: the wallet can only be funded through Paystack (bank transfer or
 * card). Crypto deposit funding is not an active rail — see
 * `src/lib/payments/active-rails.ts`.
 */
import { useEffect, useMemo, useState } from "react";
import { ArrowLeft, Check, CreditCard, Landmark, Lock, ShieldCheck, X } from "lucide-react";
...
export function AddCapitalModal({ onClose }: { onClose: () => void }) {
  ...
  const methodLabel = useMemo(
    () => localMethods.find((m) => m.id === method)?.label ?? method,
    [method],
  );
      ...
      <main className="mx-auto w-full max-w-[1100px] px-4 py-8 sm:px-6 lg:px-10 lg:py-14">
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_380px] lg:gap-14">
          <section>
            ...
            <div className="mt-5 grid grid-cols-1 gap-px overflow-hidden rounded-[10px] border border-wallet-line bg-wallet-line sm:grid-cols-2">
              {localMethods.map((m) => {
                const Icon = m.icon;
