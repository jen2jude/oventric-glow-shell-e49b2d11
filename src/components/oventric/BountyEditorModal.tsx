      ...
      <div ref={panelRef} className="relative max-h-full w-full max-w-2xl overflow-y-auto overscroll-contain rounded-[10px] border border-slate-200 bg-white p-5 shadow-xl sm:p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex flex-col">
            <h2 className="text-slate-900 font-black text-xl inline-flex items-center gap-2">
        ...
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 mb-2">
              {form.images.map((img, idx) => (
                ...
                >
                  ...
                  {idx === 0 && (
                    <span className="absolute left-1 top-1 rounded bg-red-600 px-1.5 py-0.5 text-[9px] font-bold uppercase text-slate-900">
                      Cover
                    </span>
                  )}
                  <button
                    ...
                    className="absolute right-1 top-1 rounded-[10px] bg-slate-950/75 p-1 text-slate-900 transition-colors hover:bg-red-600"
              ...
              {form.images.length < MAX_IMAGES && (
                <button
                  ...
                  className="aspect-square rounded-[10px] border border-dashed border-slate-200 hover:border-red-400 bg-slate-50 hover:bg-red-500/10 disabled:opacity-50 flex flex-col items-center justify-center gap-1 text-slate-500 hover:text-red-700 text-xs"
                >
                  {uploadingImage ? (
          ...
          <div className="flex flex-wrap gap-2 pt-3">
            <button
              disabled={saving}
              onClick={save}
              className="flex min-h-11 items-center gap-2 rounded-[10px] bg-red-600 px-6 py-3 text-sm font-black text-slate-900 shadow-lg transition-colors hover:bg-red-700 disabled:opacity-50"
            >
            ...
            <button
              type="button"
              onClick={() => saveDraft()}
              className="px-4 py-3 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-sm font-semibold rounded-[10px] inline-flex items-center gap-2"
            >
              <Save className="w-4 h-4" /> Save draft
            </button>
            <button
              onClick={onClose}
              className="px-4 py-3 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-sm font-semibold rounded-[10px]"
            >
              Cancel
            </button>
          </div>
        ...
        {showFundPrompt && (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-[10px] bg-slate-950/45 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-[10px] border border-amber-500/30 bg-white p-5 shadow-xl">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                <button
                  onClick={goToWallet}
                  className="inline-flex min-h-11 items-center gap-2 rounded-[10px] bg-red-600 px-4 py-3 text-sm font-bold text-slate-900 transition-colors hover:bg-red-700"
                >
                ...
                <button
                  ...
                  className="px-4 py-3 text-slate-600 hover:bg-slate-100 hover:text-slate-900 text-sm font-semibold rounded-[10px]"
                >
                  Back to editor
                </button>
              </div>
    ...
    >
      ...
      >
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-red-600 shadow-lg">
          <CheckCircle2 className="w-9 h-9 text-slate-900" strokeWidth={2.5} />
        </div>
        ...
        <h2 className="text-xl font-black text-slate-900 mb-1">Your bounty is in! 🎉</h2>
        <p className="text-sm text-slate-600 mb-4 leading-relaxed">
          <span className="text-slate-900 font-semibold">{title}</span> has been published and is
        ...
        <button
          onClick={onDone}
          className="mt-5 px-4 py-3 rounded-[10px] bg-slate-950 hover:bg-slate-800 text-slate-900 text-xs font-bold"
        >
          Got it
        </button>
      </div>
...
const inputCls =
  "w-full rounded-[10px] border border-slate-200 bg-white px-3 py-3 text-sm text-slate-900 placeholder:text-slate-600 outline-hidden transition-colors focus:border-red-500 focus:ring-2 focus:ring-red-500/30";
